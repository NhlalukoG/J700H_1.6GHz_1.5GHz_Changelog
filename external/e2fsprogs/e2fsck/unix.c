...
{
	fprintf(stderr,
		_("Usage: %s [-panyrcdfvtDFVe] [-b superblock] [-B blocksize]\n"
...
static errcode_t PRS(int argc, char *argv[], e2fsck_t *ret_ctx)
{
	...
	*ret_ctx = ctx;
	...
	while ((c = getopt (argc, argv, "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDke")) != EOF)
		switch (c) {
		case 'C':
			ctx->progress = e2fsck_update_progress;
...
print_unsupp_features:
	...
	if (after_resize && (sb->s_state & EXT2_ERROR_FS)) {
		log_out(ctx, _("\n ********** WARNING: Filesystem has error flag "
				"before e2fsck run **********\n"
				"\n ********** WARNING: Filesystem might be halted "
				"in resize2fs **********\n"));
		resize_err = 1;
	}
